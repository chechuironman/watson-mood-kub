...
function ready() {
  ...
  $.when(
    ...
    .done(function(thresholds, tweets, review, personalEmail, reviewFr) {
      ...
      // var texto = 'This is shit, but I love the way you dress'
      $('#transcript').val('');
      $('#table').bootstrapTable({
        onClickRow: function (row, $element) {
            ...
            $.ajax({ // we do an ajax call to the ws that will return the details
                  type: "POST",
                  url: "/metadata1",
                  data: JSON.stringify({  name:name}),//we pass the provider and the server id
                  dataType: 'json',
                      contentType: 'application/json',
                  success: function ( dataCheck){ //if the call is successfull we write teh details on the fields in the form
                    $('#transcript').val(dataCheck.name);
                    allReady(thresholds[0], sampleText, dataCheck.name);
                    //analyse(dataCheck.name);
      ...
      // allReady(thresholds[0], sampleText, texto);
    });
}
 ...
 * @return {undefined}
 */
function allReady(thresholds, sampleText, texto) {
  // var $input = $('.input'),
var    $output = $('.output'),
    $loading = $('.loading'),
    $error = $('.error'),
